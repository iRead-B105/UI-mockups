...
interface StoredVillageState {
  points: number
  owned: string[]
  placed: PlacedVillageItem[]
}
...
export function useVillageDecoration() {
  ...
  const moveItem = (itemId: string, x: number, y: number) => {
    placedItems.value = placedItems.value.map((placed) => placed.itemId === itemId
      ? { ...placed, x: clamp(x), y: clamp(y) }
      : placed)
  }
  ...
  // 화면에서 × 버튼으로 바로 지울 때: 아이템 id 로 배치 목록에서 제거.
  ...
  return { points, ownedItemIds, placedItems, placedVillageItems, selectedItem, toast, isOwned, isPlaced, purchase, beginPlacement, placeAt, moveItem, selectPlaced, removeSelected, removePlaced, cancelPlacement, showToast }
}

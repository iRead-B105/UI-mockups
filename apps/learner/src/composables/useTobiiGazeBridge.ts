...
const TRANSFORM_STORAGE_KEY = 'iread-tobii-gaze-transform-v1'
...
function readStoredTransform() {
  try {
    ...
    transform = {
      offsetX: Number.isFinite(saved.offsetX) ? Number(saved.offsetX) : Number.isFinite(legacy.x) ? Number(legacy.x) : 0,
      offsetY: Number.isFinite(saved.offsetY) ? Number(saved.offsetY) : Number.isFinite(legacy.y) ? Number(legacy.y) : 0,
      scaleX: Number.isFinite(saved.scaleX) ? Number(saved.scaleX) : 1,
      scaleY: Number.isFinite(saved.scaleY) ? Number(saved.scaleY) : 1,
    }
  } catch {
...
function setTransform(next: Partial<GazeTransform>) {
  ...
  transform = {
    offsetX: Number.isFinite(next.offsetX) ? Math.round(Number(next.offsetX)) : transform.offsetX,
    offsetY: Number.isFinite(next.offsetY) ? Math.round(Number(next.offsetY)) : transform.offsetY,
...
function fitAxis(start: GazeAnchor, end: GazeAnchor, axis: 'x' | 'y') {
  ...
  if (Math.abs(rawEnd - rawStart) < 80) return null

  const scale = (targetEnd - targetStart) / (rawEnd - rawStart)
...
function toClientPoint(frame: TobiiGazeFrame) {
  ...
  if (typeof frame.screenX === 'number' && typeof frame.screenY === 'number') {
    ...
    return {
      clientX: frame.screenX - chromeLeft,
      clientY: frame.screenY - chromeTop,
    }
  }

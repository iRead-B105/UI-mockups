...
import islandMain from '../assets/map/island-hover-base-story-889ffa-v4.png'
import growthMap from '../assets/map/island-hover-growth.png'
import gameMap from '../assets/map/island-hover-story-889ffa-v4.png'
...
<style scoped>
.map-stage{position:relative;width:min(1260px,94vw,calc((100vh - 86px) * 1.78));aspect-ratio:3/2;margin:auto;filter:drop-shadow(0 18px 20px rgba(24,111,157,.12))}.island-main,.part-image{position:absolute;inset:0;width:100%;height:100%;object-fit:contain;pointer-events:none}.island-main{z-index:3}.part-image{z-index:4;animation:region-pop var(--learner-duration-normal) var(--learner-easing-bounce) both;filter:drop-shadow(0 16px 9px rgba(39,77,101,.24))}@keyframes region-pop{from{opacity:.72;transform:translateY(0) scale(.985)}to{opacity:1;transform:translateY(-1%) scale(1.05)}}.part-growth{transform-origin:27% 49%}.part-game{transform-origin:51% 27%}.part-letter{transform-origin:75% 49%}.part-challenge{transform-origin:51% 72%}.part-hit{position:absolute;z-index:8;inset:0;width:100%;height:100%;border:0;outline:none;background:transparent;cursor:pointer}.hit-growth{clip-path:polygon(5% 25%,36% 21%,47% 48%,38% 76%,20% 92%,5% 73%)}.hit-game{clip-path:polygon(28% 12%,69% 12%,72% 37%,60% 55%,44% 58%,30% 46%)}.hit-letter{clip-path:polygon(63% 27%,89% 27%,96% 51%,88% 74%,67% 77%,57% 52%)}.hit-challenge{clip-path:polygon(28% 50%,57% 52%,73% 72%,67% 91%,47% 97%,24% 86%,17% 68%)}.part-hit:focus-visible{box-shadow:inset 0 0 0 6px rgba(255,255,255,.95);filter:drop-shadow(0 0 8px var(--learner-home-sea))}@media(max-width:820px){.map-stage{width:min(1150px,138vw);left:-18vw}}@media(prefers-reduced-motion:reduce){.part-image{animation:none;transform:translateY(-1%) scale(1.05)}}
</style>

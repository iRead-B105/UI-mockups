...
import {
  fetchLinkedStudents,
  saveTeacherBootstrapSession,
  saveSelectedStudentSession,
} from '@/services/learnerDataRepository'
import type { MockLinkedStudent } from '@/mocks/learnerRuntimeMock'

const router = useRouter()
const loginStep = ref<'teacher' | 'student'>('teacher')
const loginId = ref('')
const password = ref('')
const linkedStudents = ref<MockLinkedStudent[]>([])
...
const handleTeacherLogin = async () => {
  ...
  try {
    // TODO: POST /api/auth/app/teacher-login 응답의 teacherSessionToken과
    // linkedStudents로 아래 목업 값을 교체합니다.
    saveTeacherBootstrapSession()
    linkedStudents.value = await fetchLinkedStudents()
    loginStep.value = 'student'
  } catch {
    errorMessage.value = '교수자 계정을 확인해 주세요.'
  } finally {
...
const handleStudentLogin = async () => {
  ...
  try {
    // TODO: POST /api/auth/app/student-login으로 선택 아동 범위의 access token과
    // learningRefreshToken cookie를 발급받도록 교체합니다.
    saveSelectedStudentSession(student)
    await router.push({ name: 'learner-home' })
  } catch {
    errorMessage.value = '아동 학습 세션을 시작할 수 없습니다.'
  } finally {
    isLoading.value = false
  }
}

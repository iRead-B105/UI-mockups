...
<style scoped>
.story-select-page { position:relative;display:grid;place-items:center;width:100%;height:100%;min-height:0;overflow:hidden;padding:clamp(10px,1.8vh,24px) var(--learner-page-padding);background:var(--learner-background);color:var(--learner-color-text);font-family:var(--learner-font-reading); }
...
.story-select-panel { position:relative;z-index:1;display:flex;flex-direction:column;align-items:center;justify-content:flex-start;gap:clamp(10px,1.7vh,22px);width:min(100%,1500px);height:min(100%,830px);min-height:0;padding:clamp(14px,2vh,28px) clamp(24px,3.4vw,58px) clamp(16px,2.2vh,28px);border-radius:var(--learner-radius-card);background:color-mix(in srgb,var(--learner-color-game) 8%,var(--learner-color-surface));box-shadow:var(--learner-shadow-floating); }
...
@media (max-height:760px) { .story-select-page { padding-block:var(--learner-space-2); }.story-select-panel { height:100%;min-height:0;padding-block:var(--learner-space-3); }.story-select-heading h1 { font-size:clamp(34px,4vw,54px); } }
</style>

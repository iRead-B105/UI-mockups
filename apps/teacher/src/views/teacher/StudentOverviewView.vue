...
const draftsByStudent = reactive<Record<number, string>>(
...
const noteDraft = computed({
  get: () => draftsByStudent[currentStudent.value.id] ?? '',
  set: (value: string) => {
    draftsByStudent[currentStudent.value.id] = value
  },
})
...
<template>
  <div class="overview page-stack">
    ...
    <Card class="student-facts" aria-label="아동 학습 상태 요약">
      ...
      <div class="action-summary" :class="{ 'is-complete': reviewCount === 0 }">
        <div v-if="reviewCount > 0">
          <strong>확인할 항목 {{ reviewCount }}건</strong>

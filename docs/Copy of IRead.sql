CREATE TABLE `stories` (
	...
	`last_read_at`	timestamp	NULL	COMMENT '아동이 마지막으로 이야기 페이지를 읽은 시각',
	`progress`	int	NOT NULL	DEFAULT 0	COMMENT '읽기 진행률 0~100',
...
CREATE TABLE `story_templates` (
	...
	`cover_image_url`	varchar(255)	NOT NULL	COMMENT '새로운 책 읽기 목록에 표시할 초기 표지',
...
CREATE TABLE `word_attempt_logs` (
	...
	`story_line_id`	long	NULL,
	`training_id`	long	NULL,
	`test_id`	long	NULL,
	`use_location`	varvhar(10)	NOT NULL,
	...
	`attempt_no`	int	NULL	COMMENT '같은 문항 안에서의 시도 순서',
	`hint_used`	boolean	NOT NULL	DEFAULT false	COMMENT '해당 시도 전에 자동 시각 힌트를 사용했는지',
	`tts_playback_count`	int	NOT NULL	DEFAULT 0	COMMENT '해당 항목의 정답 소리/TTS 누적 재생 횟수',
	`created__at`	timestamp	NULL,
	`total_score`	int	NULL	COMMENT '지표를 통해 계산한 단어의 점수 0~1000'
);
...
CREATE TABLE `story_choices` (
	`id`	long	NOT NULL,
	`story_line_id`	long	NOT NULL,
	`content`	text	NOT NULL,
	`audio_url`	varchar(255)	NOT NULL	COMMENT '아동 분기 응답 원본 음성 저장 위치/API 응답 비노출',
	`created_at`	timestamp	NOT NULL
);
...
CREATE TABLE `story_generation_jobs` (
	`id`	long	NOT NULL,
	`story_id`	long	NOT NULL,
	`story_line_id`	long	NOT NULL	COMMENT '분기 질문이 발생한 원본 장면',
	`story_choice_id`	long	NOT NULL	COMMENT '저장된 아동 원본 음성 및 STT 답변',
	`result_story_line_id`	long	NULL	COMMENT '생성 완료된 다음 장면 또는 엔딩 장면',
	`status`	varchar(20)	NOT NULL	COMMENT 'Enum: QUEUED, RUNNING, COMPLETED, FAILED',
...
CREATE TABLE `daily_curriculums` (
	`id`	long	NOT NULL,
	`student_id`	long	NOT NULL,
	`status`	varchar(20)	NOT NULL	COMMENT 'Enum: PREPARING, READY, IN_PROGRESS, COMPLETED, REST',
	`created_at`	timestmap	NOT NULL,
	`completed_at`	timestamp	NULL	COMMENT '회차의 실제 학습 완료 시각. 이 날짜를 학습일로 사용'
);
...
CREATE TABLE `character` (
	`id`	long	NOT NULL,
	`student_id`	long	NOT NULL,
	`story_id`	long	NOT NULL	COMMENT '친구를 획득한 생성 이야기 세션',
	`name`	varchar(50)	NOT NULL	COMMENT '이야기 엔딩에서 AI가 생성한 친구 이름',
	`image_url`	varchar(255)	NULL,
	`image_status`	varchar(20)	NOT NULL	DEFAULT 'READY'	COMMENT 'Enum: GENERATING, READY, FAILED',
	`created_at`	timestamp	NOT NULL
);
...
CREATE TABLE `trainings` (
	...
	`result`	json	NULL	COMMENT '문항별 정답유무/틀린부분',
	`accuracy`	int	NULL	COMMENT 'AI가 훈련결과JSON으로 판단한 정확도(0~1000)'
);
...
ALTER TABLE `story_generation_jobs` ADD CONSTRAINT `PK_STORY_GENERATION_JOBS` PRIMARY KEY (
	`id`
);
